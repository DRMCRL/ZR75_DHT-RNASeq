...
plotly::ggplotly(
  counts %>%
    ...
    left_join(samples) %>%
    ggplot(aes(group, Detected, colour = group, label = Filename)) +
    geom_point() +
    geom_segment(
      aes(x = group, xend = group, y = 0, yend = Detected),
      data = . %>%
        group_by(group) %>%
        summarise(Detected = min(Detected), .groups = "drop"),
...
**The above plot has clearly highlighted the replicates associated with `SLX-7489.D712_D503` as being low quality in terms of the number of genes detected.**
...
**The above plot has clearly highlighted the replicates associated with `SLX-7489.D712_D503` as being low quality in terms of library sizes.**
...
**The libraries associated with `SLX-7489.D712_D503` are shown spread relatively widely in the bottom left, whilst all other split samples cluster very closely, again confirming concerns with this sample**
