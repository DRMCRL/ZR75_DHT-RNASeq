...
The value of $\lambda = \log_2 `r 2^lambda` = `r round(lambda, 3)`$ was chosen, which is **less stringent than the default value of `r round(log2(1.2), 3)`**, but still adds a degree of robustness to the results.
...
topTable %>%
  ...
  geom_text_repel(
    aes(label = gene_name, colour = DE),
    data = . %>%
      dplyr::filter(PValue < 1e-6 | (DE & logFC < 0)),
    show.legend = FALSE
  ) +
...
logCPM %>%
  ...
  as.data.frame() %>%
  rownames_to_column("gene_id") %>%
  mutate(gene_id = factor(gene_id, levels = up)) %>%
  pivot_longer(
    cols = starts_with("ZR"),
    names_to = "name",
    values_to = "logCPM"
...
```{r plotTopDown, fig.height=8, fig.width=6, echo=FALSE, fig.cap = "*Top ranked down-regulated genes*"}
logCPM %>%
  .[head(down, 12),] %>%
  as.data.frame() %>%
  rownames_to_column("gene_id") %>%
  mutate(gene_id = factor(gene_id, levels = down)) %>%
  pivot_longer(
    cols = starts_with("ZR"),
    names_to = "name",
    values_to = "logCPM"
...
The logCPM values were also exported
...
logCPM %>%
  ...
  rownames_to_column("gene_id") %>%
  left_join(
    mcols(genesGR) %>% as.data.frame()
  ) %>%
  as_tibble() %>%
  dplyr::select(
    gene_id, gene_name, starts_with("ZR")
  ) %>%
  write_tsv(
    here::here("output", "ZR75_DHT-logCPM.tsv")
  )
```
