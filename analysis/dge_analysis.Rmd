...
**Importantly**, this dataset has a different structure to the 453 and 223 datasets, in that the *strict pairing of replicates between Vehicle & DHT-treated is not present.*
As such, the same workflow **cannot** be followed and an accompanying loss of power for detection of DE genes is to be expected.
...
```{r plotDensities, fig.height=5, fig.cap="*Distributions of logCPM values for merged counts, A) before and B) after filtering of undetectable genes. Some differences between replicates were noted.*"}
a <- mergedCounts %>%
  cpm(log = TRUE) %>%
  as.data.frame() %>%
  rownames_to_column("gene_id") %>%
...
```{r plotPCA, fig.height=8, fig.width=8, fig.cap="*PCA on logCPM from merged counts. Only two replicates have paired samples across both treatments and only three Vehicle samples were identified. Some separation between treatment groups is evident across PC2 & PC3*"}
a <- pca %>%
  autoplot(
    data = dge$samples, 
    x = 1, y = 3,
...
```{r plotCQN, fig.height=6, fig.cap = "*Model fits used when applying CQN. The clear relationship between the two pairs which are present is in keeping with the previous PCA analysis.*"}
a <- cqNorm$func1 %>%
  as.data.frame() %>%
...
```{r plotPcaPost, fig.height=8, fig.width=8, fig.cap="*PCA on logCPM after performing CQN. As with the data before normalisation, PC2 and PC3 appear to capture the majority of the biological signal*"}
a <- pcaPost %>%
  autoplot(
    data = dge$samples, 
    x = 1, y = 3,
...
As the strict paring between replicates was not present in this dataset, the fitted effect was simply that of DHT treatment.
Dispersions were estimated for the main `DGEList` object and the model fitted using the Quasi-likelihood GLM.
...
topTable %>%
  ...
  geom_text_repel(
    ...
    data = . %>%
      dplyr::filter(logFC > 1.8 | logFC < -1),
    show.legend = FALSE
  ) +
...
topTable %>%
  ...
  geom_text_repel(
    aes(label = gene_name, colour = DE),
    data = . %>%
      dplyr::filter(abs(logFC) > 1.4 | PValue < 1e-4),
    show.legend = FALSE
  ) +
...
```{r plotTopDown, fig.height=8, echo=FALSE, fig.cap = "*Top ranked down-regulated genes. None were formally considered as DE*"}
topTable %>%
  dplyr::filter(logFC < 0) %>%
  dplyr::slice(1:12) %>%
  dplyr::select(gene_id, gene_name) %>%
...
These above results were exported as the file `ZR75_DHT_RNASeq.tsv`.
...
topTable %>%
  ...
  write_tsv(
    file = here::here("output/ZR75_DHT_RNASeq.tsv")
  )
```
